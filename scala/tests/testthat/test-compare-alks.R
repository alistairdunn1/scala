test_that("compare_alks basic functionality", {
  ...
  expected_names <- c(
    "comparison_data", "summary_stats", "empirical_type",
  ...
  expected_cols <- c(
    "length", "sex", "age", "empirical_prop", "model_prop",
...
test_that("compare_alks summary statistics", {
  ...
  expected_summary_names <- c(
    "rmse_overall", "mae_overall", "max_abs_diff",
  ...
  for (sex in names(summary_stats$rmse_by_sex)) {
    ...
    # Check if correlation exists and is not NA
    if (!is.null(summary_stats$correlation_by_sex[[sex]]) && !is.na(summary_stats$correlation_by_sex[[sex]])) {
